...
//gli
procedure KDTreeCreateRequestBuffer(const KDT : KDTree;
      var RB: KDTreeRequestBuffer);
function KDTreeSQueryL2KNNTag(
     var RB: KDTreeRequestBuffer;
     const X : TReal1DArray):AlglibInteger;
...
GliGli's addons for thread safety (inspired by ALGLIB 3.0 C++)
...
begin
  ...
  RB.KNeeded := KDT.KNeeded;
  RB.RNeeded := KDT.RNeeded;
  RB.SelfMatch := KDT.SelfMatch;
  RB.ApproxF := KDT.ApproxF;
  RB.KCur := KDT.KCur;
  RB.CurDist := KDT.CurDist;


  RB.X := DynamicArrayCopy(KDT.X);
  RB.BoxMin := DynamicArrayCopy(KDT.BoxMin);
  RB.BoxMax := DynamicArrayCopy(KDT.BoxMax);
  RB.CurBoxMin := DynamicArrayCopy(KDT.CurBoxMin);
  RB.CurBoxMax := DynamicArrayCopy(KDT.CurBoxMax);
  RB.Idx := DynamicArrayCopy(KDT.Idx);
  RB.R := DynamicArrayCopy(KDT.R);
  RB.Buf := DynamicArrayCopy(KDT.Buf);
end;

procedure KDTreeSInitBox(var RB : KDTreeRequestBuffer;
      const X : TReal1DArray);
var
    I : AlglibInteger;
    NX: AlglibInteger;
...
begin
    ...
    begin
        ...
        if AP_FP_Less(VX,VMin) then
        begin
            RB.CurDist := RB.CurDist+AP_Sqr(VMin-VX);
        end
        else
        begin
            if AP_FP_Greater(VX,VMax) then
            begin
                RB.CurDist := RB.CurDist+AP_Sqr(VX-VMax);
            end;
        end;
        Inc(I);
    end;
...
procedure KDTreeSQueryNNRec(var RB: KDTreeRequestBuffer;
      Offs : AlglibInteger);
var
    PtDist : Double;
    I : AlglibInteger;
...
begin
    ...
    // Process points.
    //
    if RB.KDT^.Nodes[Offs]>0 then
    begin
        I1 := RB.KDT^.Nodes[Offs+1];
        I2 := I1+RB.KDT^.Nodes[Offs];
        I:=I1;
        while I<=I2-1 do
        begin
            ...
            while J<=NX-1 do
            begin
                PtDist := PtDist+AP_Sqr(XYD[J]-RB.X[J]);
                Inc(J);
            end;
            ...
            // i.e. (RNeeded<>0) AND (PtDist>R).
            //
            if AP_FP_Eq(RB.RNeeded,0) or AP_FP_Less_Eq(PtDist,RB.RNeeded) then
            begin
                ...
                // * add point without replacement otherwise
                //
                if (RB.KCur<RB.KNeeded) or (RB.KNeeded=0) then
                begin

                    //
                    // add current point to heap without replacement
                    //
                    TagHeapPushI(RB.R, RB.Idx, RB.KCur, PtDist, I);
                end
                else
                begin
                    ...
                    // If added, they replace element at the top of the heap
                    //
                    if AP_FP_Less(PtDist,RB.R[0]) then
                    begin
                        if RB.KNeeded=1 then
                        begin
                            RB.Idx[0] := I;
                            RB.R[0] := PtDist;
                        end
                        ...
                        begin
                            TagHeapReplaceTopI(RB.R, RB.Idx, RB.KNeeded, PtDist, I);
                        end;
                    end;
                end;
            end;
    ...
    begin
        ...
        // * S  split position
        //
        D := RB.KDT^.Nodes[Offs+1];
        S := RB.KDT^.Splits[RB.KDT^.Nodes[Offs+2]];
        ...
        // * ChildWorstOffs     child box with worst chances
        //
        if AP_FP_Less_Eq(RB.X[D],S) then
        begin
            ChildBestOffs := RB.KDT^.Nodes[Offs+3];
            ChildWorstOffs := RB.KDT^.Nodes[Offs+4];
            BestIsLeft := True;
        end
        else
        begin
            ChildBestOffs := RB.KDT^.Nodes[Offs+4];
            ChildWorstOffs := RB.KDT^.Nodes[Offs+3];
            BestIsLeft := False;
        end;
        ...
        begin
            ...
            begin
                ...
                if AP_FP_Less_Eq(T1,S) then
                begin
                    RB.CurDist := RB.CurDist-AP_Sqr(Max(V-T1, 0))+AP_Sqr(S-T1);
                end;
                RB.CurBoxMin[D] := S;
            end
            else
            begin
                ...
                if AP_FP_Greater_Eq(T1,S) then
                begin
                    RB.CurDist := RB.CurDist-AP_Sqr(Max(T1-V, 0))+AP_Sqr(T1-S);
                end;
                RB.CurBoxMax[D] := S;
            end;
            ...
            // Decide: to dive into cell or not to dive
            //
            if AP_FP_Neq(RB.RNeeded,0) and AP_FP_Greater(RB.CurDist,RB.RNeeded) then
            begin
                ToDive := False;
            end
            else
            begin
                ...
                begin
                    ...
                    // using point position relative to bounding box.
                    //
                    ToDive := AP_FP_Less_Eq(RB.CurDist,RB.R[0]*RB.ApproxF);
                end;
            end;
            if ToDive then
            begin
                KDTreeSQueryNNRec(RB, ChildOffs);
            end;
...
var
    ...
    J : AlglibInteger;
begin
    Assert(RB.KDT^.NormType = 2, 'KDTreeQueryKNN: NormType <> 2 !');
    ...
    while I>=2 do
    begin
        TagHeapPopI(RB.R, RB.Idx, J);
        Dec(I);
    end;

    // gli: directly return tag
    Result := RB.KDT^.Tags[RB.Idx[0]];
end;
